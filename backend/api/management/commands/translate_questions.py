...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f'LangChain init'))
            test_response = llm.invoke("test")
            self.stdout.write(self.style.SUCCESS(f'Successful connection to Ollama'))
...
- 18: Extreme violence, explicit sexual content, hard drugs, strong encouragement of gambling
...
"""
                ...
                try:
                    ...
                    if json_start != -1 and json_end > json_start:
                        ...
                        if not question_ru or not answer_ru:
                            self.stdout.write(self.style.WARNING(f'Empty translation received!!!!!'))
                            error_count += 1
                            continue

                        valid_pegi = ['3', '7', '12', '16', '18']
                        if pegi_rating not in valid_pegi:
                            self.stdout.write(self.style.WARNING(f'Invalid PEGI "{pegi_rating}" (default to "7")'))
                            pegi_rating = '7'
                        ...
                        self.stdout.write(self.style.WARNING(f'No valid JSON found in response'))
        ...
        translated_total = Question.objects.filter(translation_text__isnull=False).count()
        ...
        self.stdout.write(self.style.SUCCESS(
            ...
            f'Technology Stack:\n'
            f'  - LangChain: Prompt engineering & chains\n'
            f'  - Ollama: Inference engine\n'
            f'  - Model: {model_name} (quantized Llama/Qwen from HuggingFace)\n'
            f'\n'
            f'This Session:\n'
